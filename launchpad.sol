...
contract MemecoinLaunchpad is ERC20, Ownable, ReentrancyGuard {
    ...
    function buyTokens(uint256 tokenAmount, address referrer) external payable nonReentrant {
    ...
    function setSocialMediaAccounts(string memory telegram, string memory twitter) external {
        socialMediaAccounts[msg.sender] = SocialMedia(telegram, twitter);
    }

    // Function to get social media accounts
    function getSocialMediaAccounts(address user) external view returns (string memory, string memory) {
